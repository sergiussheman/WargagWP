...
namespace Wargag.ViewModels
{
    public class MainViewModel : INotifyPropertyChanged
    {
        ...
        private RootObject MainRootObject;
        private string currentURLRequest;
        private List<Datum> _Quotes = new List<Datum>();
        private List<Datum> _Pictures = new List<Datum>();
        private List<Datum> temp;

        public MainViewModel()
        {
            this.Items = new ObservableCollection<ItemViewModel>();
            this.currentURLRequest = UtilityClass.URLRequest;
        }
        ...
        public Datum findQuote(string descript)
        {
            return Quotes.Find(x => x.description == descript);
        }
        ...
        public void LoadInfo()   //выполнение запроса для скачивания информации с сайта
        {
            ...
            currentURLRequest = UtilityClass.URLRequest;
        ...
        private void webClient_DownloadStringCompleted(object sender, DownloadStringCompletedEventArgs e)
        {
            ...
            {
                MainRootObject = JsonConvert.DeserializeObject<RootObject>(e.Result); //парсинг полученного ответа

                 temp = MainRootObject.data;
                 ParsingAnswer();
            }
        }
        ...
        private void ParsingAnswer()
        {
            ...
            foreach (var item in temp)
            {
                ...
                else
                {
                    ...
                    {
                        tempPictures.Add(item);
                    }   
                }

                if (item.rating < 0)
                    item.BackgroundColor = new SolidColorBrush(Colors.Red);
                else item.BackgroundColor = new SolidColorBrush(Colors.Green);
        ...
        private string FormatText(string text) //удалением html-тегов из цитат
        {
